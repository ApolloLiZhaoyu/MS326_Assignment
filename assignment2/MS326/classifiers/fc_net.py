    ...
    def loss(self, X, y=None):
        ...
        for i in range(hidden_num):
            if self.normalization == 'batchnorm':
                ...
                scores, cache = affine_ln_relu_forward(scores, self.params['W%d' % (i + 1)], self.params['b%d' % (i + 1)], self.params['gamma%d' % (i + 1)], self.params['beta%d' % (i + 1)], self.ln_params[i])
            else:
