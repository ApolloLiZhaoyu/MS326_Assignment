...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    scores[range(N), y] -= 1
    grads['W2'] = relu.T.dot(scores) / relu.shape[0]
    grads['W2'] += 2 * reg * W2
    grads['b2'] = np.ones(N).dot(scores) / relu.shape[0]

    grads['W1'] = np.zeros_like(W1)
    relu[relu > 0] = 1
    scores = scores.dot(W2.T) * relu
    grads['W1'] = X.T.dot(scores) / N
    grads['W1'] += 2 * reg * W1
    grads['b1'] = np.ones(N).dot(scores) / N
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
